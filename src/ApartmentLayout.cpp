...
[[maybe_unused]] ApartmentLayout::ApartmentLayout(const ApartmentLayout &other_obj): Layout(other_obj), max_rows(other_obj.max_rows), max_columns(other_obj.max_columns) {
    ...
    for(int i = 0; i < max_rows; i++)
    {
        ...
        {
            m_layout[i][j] = other_obj.m_layout[i][j];
        }
    }
}
...
void ApartmentLayout::display(std::ostream &out) const {
    ...
    {
        //auto ptr_room = dynamic_cast<RoomLayout *>(item);
...
void ApartmentLayout::setMatrixLayout(int i, int j, const std::string &name) {
    if((unsigned long long)i < this->m_layout.size() && (unsigned long long)j < this->m_layout[i].size())
    {
        m_layout[i][j] = name;
    }
}

void ApartmentLayout::displayFloorPlan() {
