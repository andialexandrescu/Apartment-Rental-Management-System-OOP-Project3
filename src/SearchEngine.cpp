...
SearchEngine &query = SearchEngine::getTimeline(); // singleton principle
...
std::ostream &operator<<(std::ostream &out, const SearchEngine &obj) {
    std::cout<<"------- Search Results -------"<<std::endl;
    for(const Apartment &apt: obj.l_apts)
    {
        out<<apt<<std::endl;
    }
    return out;
...
SearchEngine &SearchEngine::getTimeline() {
    static SearchEngine curr_timeline;
    return curr_timeline;
}

void SearchEngine::searchResults() {
    ...
    std::cin>>p;
    if (query.l_apts.empty())
    {
        std::cout<<"No apartments available."<<std::endl;
        return;
    }
    for(const Apartment& apt : query.l_apts)
    {
...
void SearchEngine::proceedTransaction(bool &booked) { // NOLINT(*-no-recursion)
    int choice;
    std::string apt_id;
    searchResults();
    ...
    switch(choice)
    {
        case 1:
            ...
            std::cin>>apt_id;
            for([[maybe_unused]] auto apt: query.l_apts)
            {
